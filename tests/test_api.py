...
TEST_SHORT_LINK_NAME = sha256(TEST_URL.encode()).hexdigest()[:8]
...
def test_get_new_link_from_db_custom(client):
    response = client.get(f'/{TEST_LINK_NAME}')
    assert '302 FOUND' == response.status


def test_get_not_existed_link_db_custom(client):
    response = client.get(f'/{TEST_LINK_NAME}dsaeqw')
...
def test_write_link_db_auto(client):
    response = client.get(f'/api/make-short?link={TEST_URL}')
    assert TEST_SHORT_LINK_NAME in response.json['message']


def test_get_link_db_auto(client):
    response = client.get(f'/{TEST_SHORT_LINK_NAME}')
    assert '302 FOUND' == response.status


def test_get_not_existed_link_db_auto(client):
    response = client.get(f'/{TEST_SHORT_LINK_NAME}llsdd123124fdsar')
    assert Responses.NO_SUCH_SHORT_LINK == response.json
